...
class OTOS {
  ...
  public:
     ...
     *
     * @param samples Number of samples to take for calibration. Each sample
     * takes about 2.4ms. Fewer samples means faster calibration, but reduced
     * accuracy. 255 by default.
     * @param blocking Whether the function should block until calibration has
     * been completed. true by default.
     *
     * @return void on success
     * @return Error<ErrorType> on failure
     */
    std::expected<void, Error<ErrorType>> calibrate_imu(uint8_t samples = 255,
                                                        bool blocking = true);
    /**
     ...
     *
     * Can be used to compensate for scaling issues with the sensor
     * measurements.
     *
    ...
    /**
     * @brief Get the status of the OTOS, which includes warning and errors
